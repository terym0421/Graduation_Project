...
\begin{cor}\label{multiplicity}
  ...
  \[
  V\simeq \bigoplus_{i=1}^rW_i\otimes \Hom_G(W_i,V) =\bigoplus_{i=1}^rW_i\otimes \Hom_G(V,W_i)  
  \]
  がなりたつ。
\end{cor}

\begin{proof}
  系\ref{irr_char_is_basis}より前半は直ちに従う。後半については、$\Hom_G(W_i,V)$は定義から自明な表現の直和に等しいことからわかる。また、$\generated{\chi_{W_i},\chi_V}$は整数であるから
  \[
  dim_\complex\Hom_G(W_i,V)=\generated{\chi_{W_i},\chi_V}=\generated{\chi_V,\chi_{W_i}}=\dim_\complex\Hom_G(V,W_i)
  \]
  よって
  \[
  \Hom_G(W_i,V)\simeq\Hom_G(V,W_i)  
  \]
  である。
\end{proof}
