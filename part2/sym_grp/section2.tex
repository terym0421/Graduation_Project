...
\]
によって定める。ここで$\rho_{m,n}(f)$は実際に$m$変数の$k$次斉次対称多項式である\footnote{変数の置換と$0$を代入する写像が可換であることによる}。$l\leq m\leq n$に対して
\[
...
\begin{proof}
  $\Lambda^k$が射影極限の普遍性を満たすことを言えばよい。
  線形写像$\map{\rho_n}{\Lambda^k}{\Lambda^k_n}$を
  \[
  \rho_n(f(x_1,x_2,\cdots))=f(x_1,x_2,\cdots,x_n,0,0,\cdots)  
  \]
  によって定義すれば$m\leq n$に対して
  \[
  \rho_{m,n}\circ\rho_{n}=\rho_m
  \]
  が成り立つ。任意のベクトル空間$V$と線形写像$\phi_n:V\rightarrow\Lambda^k_n$であって$m\leq n$に対して
  \[
  \rho_{m,n}\circ\phi_n=\phi_m  
  \]
  を満たすものが与えられたとき、$\varphi:V\rightarrow\Lambda^k$を
  \[
  \varphi(x)=  
  \]
\end{proof}
