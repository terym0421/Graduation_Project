...
対称関数の$k$次斉次部分は$k$次斉次対称多項式からの射影極限と考えることができる。$\Lambda_n^k$を$n$変数$k$次斉次対称多項式のなすベクトル空間とする。$m\leq n$に対して線形写像$\map{\rho_{m,n}}{\Lambda^k_n}{\Lambda^k_m}$を
\[
...
\begin{proof}
  ...
  \]
  が誘導される。$\mathop{\lim_{\longleftarrow}}\Lambda^k_n$から$\Lambda^k$への写像は次のように定義する。$\mathop{\lim_{\longleftarrow}}\Lambda^k_n$の元$(f_n)_n$, $f_n\in\Lambda^k_n$に対して、
\end{proof}
...
このように、対称関数はいままでみてきた対称多項式を自然に無限変数に拡張した概念であり、対称多項式で成り立っていた関係式が対称関数においても成立することが多い。
