...
\subsection{続・対称多項式}
第1部で述べた対称多項式について、本節でもう少し詳しく説明する。
基本的な対称多項式の例として次のようなものがあった。
\begin{itemize}
  \item 大きさ$k$の$n$行のYoung図形$\lambda\in\mathcal{Y}_{k,n}$に対して
  \[
  m_\lambda=\sum_{\mu\sim\lambda}x_1^{\lambda_1}\cdots x_n^{\lambda_n}  
  \] 
  ただし$\lambda\sim\mu$とはある$\sigma\in\mathfrak{S}_n$に対して$(\mu_1,\cdots,\mu_n)=(\lambda_{\inv{\sigma}(1)},\cdots,\lambda_{\inv{\sigma}(n)})$が成り立つことである\footnote{したがって$\mu$はYoung図形ではない}。

  \item $1^k=(1,1,\cdots,1,0,0,\cdots,0)\in\mathcal{Y}_{k,n}$に対して
  \[
  e_k=m_{1^k}  
  \]
