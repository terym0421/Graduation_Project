...
\begin{eg}\label{monomial_sym_func}
  ...
  \]
  とする。ここで指数$\alpha$は、$\lambda$の置換になっているもの全体をわたる。すなわちある$\sigma\in\mathfrak{S}$が存在して$\alpha=\sigma\lambda$をみたすもの全体である。$m_\lambda$は対称関数である。
\end{eg}

\begin{eg}
  \begin{align*}
  &e_k=m_{1^k}=\sum_{1\leq i_1<i_2<\cdots<i_k}x_{i_1}x_{i_2}\cdots x_{i_k}\\
  &h_k=\sum_{\lambda\in\mathcal{P}_n} m_\lambda
  \end{align*}
...
このように、対称関数はいままでみてきた対称多項式を自然に無限変数に拡張した概念であり、対称多項式で成り立っていた関係式が対称関数においても成立することが多い。このことは対称関数の$k$次斉次部分が$k$次斉次対称多項式からの射影極限と考えることができることによる。$\Lambda_n^k$を$n$変数$k$次斉次対称多項式のなすベクトル空間とする。$m\leq n$に対して線形写像$\map{\rho_{m,n}}{\Lambda^k_n}{\Lambda^k_m}$を
\[
...
\begin{proof}
  ...
  \]
  が誘導される。$\mathop{\lim_{\longleftarrow}}\Lambda^k_n$から$\Lambda^k$への写像$\varphi$は次のように定義する。$\mathop{\lim_{\longleftarrow}}\Lambda^k_n$の元$(f_n)_{n\in\integer_{>0}}$, ($f_n\in\Lambda^k_n$)に対して、$f_n\neq 0$となる$f_n$をとる。$f_n$は$n$変数$k$次斉次対称多項式だから、大きさ$k$の$n$行Young図形$\mathcal{P}_k(n)$でパラメトライズされた$\{m_\lambda(x_1,\cdots,x_n)\}_{\lambda\in\mathcal{P}_k(n)}$で生成される。すなわち
  \[
  f_n=\sum_{\lambda\in\mathcal{P}_k(n)}c_\lambda m_\lambda(x_1,\cdots,x_n)  
  \]
  と表される。そこで、
  \[
  \varphi((f_n)_{n\in\integer_{>0}})=  \sum_{\lambda\in\mathcal{P}_k(n)}c_\lambda m_\lambda
  \]
  によって定義する。ただし、右辺の$m_\lambda$は例\ref{monomial_sym_func}の対称関数である。$\varphi$が$f_n$の取り方によらないことを示す。$f_m\neq 0$かつ$m\geq n$とする。
  \[
  f_m=\sum_{\mu\in\mathcal{P}_k(m)}d_\mu m_\mu(x_1,\cdots,x_n,x_{n+1},\cdots,x_m)  
  \]
  と表せば、$\rho_{n,m}(f_m)=f_n$より
  \begin{equation}\label{equality_1}
    \sum_{\lambda\in\mathcal{P}_k(n)}c_\lambda m_\lambda(x_1,\cdots,x_n) 
    =
    \sum_{\mu\in\mathcal{P}_k(m)}d_\mu m_\mu(x_1,\cdots,x_n,0,\cdots,0)  
  \end{equation}
  となる。ここで、$\mu_{n+1}>0$となる$\mu$に対しては$m_\mu(x_1,\cdots,x_n,0,\cdots,0)=0$となるから、式(\ref{equality_1})の右辺の和で残るのは、$\mu_{n+1}=0$なる$\mu$すなわち$\mu\in\mathcal{P}_k(n)$である。また、$\mu\in\mathcal{P}_k(n)$のとき、
  \[
  m_\mu(x_1,\cdots,x_n,0,\cdots,0)=m_\mu(x_1,\cdots,x_n)  
  \]
  であるから、結局式(\ref{equality_1})は
  \begin{equation}
    \sum_{\lambda\in\mathcal{P}_k(n)}c_\lambda m_\lambda(x_1,\cdots,x_n) 
    =
    \sum_{\lambda\in\mathcal{P}_k(n)}d_\lambda m_\lambda(x_1,\cdots,x_n) 
  \end{equation}
  となり、$c_\lambda=d_\lambda$が従う。
\end{proof}
